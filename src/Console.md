# Console
---
你有没有意识到：你在学校学习的思科模拟器的IOS命令行，只需要点几下图标就有了。真机你鼠标点不动。
...
### 4. 华为设备，插上console直接输命令，没提示输密码

**原因**：华为默认console口不需要认证。  
这是厂商差异。思科默认要密码，华为默认不要。第一次连上华为设备，你可能直接就进用户视图了，反而会愣一下。
...
| 华为交换机/路由器 | 默认无密码，直接进 | 进用户视图后需要`system-view`进配置模式 |
